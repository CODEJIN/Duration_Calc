...
class Logger(SummaryWriter):
        ...
        for tag, (data, size, aspect, x_limit, y_limit, c_limit) in image_dict.items():
            ...
            if data.ndim == 1:
                ...
                if not y_limit is None:
                    plt.ylim(*y_limit)
            elif data.ndim == 2 or (data.ndim == 3 and data.shape[2] in [3, 4]):
    ...
    def add_text_dict(self, text_dict, global_step, walltime= None):
        for tag, text in text_dict.items():
            self.add_text(tag= tag, text_string= text, global_step= global_step, walltime= walltime)
        self.flush()
